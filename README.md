...
```bash
discord-video-webhook-api$ sam build
```
...
```bash
discord-video-webhook-api$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
discord-video-webhook-api$ sam local start-api
discord-video-webhook-api$ curl http://localhost:3000/
```
...
```bash
discord-video-webhook-api$ sam build
discord-video-webhook-api$ sam deploy
```
...
```bash
discord-video-webhook-api$ sam logs -n NetCodeWebAPIServerless --stack-name discord-video-webhook-api --tail
```
...
```bash
discord-video-webhook-api$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name discord-video-webhook-api
```
